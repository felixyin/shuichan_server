...
<%@ attribute name="addUrl" type="java.lang.String" required="false" description="新增数据地址" %>
...
<%@ attribute name="callback" type="java.lang.String" required="false" description="点击确定后回调函数，设置为disabled" %>
    ...
    $(document).ready(function () {
        $("#${id}Button, #${id}Name").click(function () {
            ...
            top.layer.open({
                ...
                auto: true,
                name: 'friend',
                content: "${ctx}/tag/gridselect?url=" + encodeURIComponent("${url}")+ "&addUrl=" + encodeURIComponent("${addUrl}") + "&fieldLabels=" +
						encodeURIComponent("${fieldLabels}") + "&fieldKeys=" + encodeURIComponent("${fieldKeys}") + "&searchLabels=" + encodeURIComponent("${searchLabels}") + "&searchKeys=" + encodeURIComponent("${searchKeys}") + "&isMultiSelected=${isMultiSelected? true:false}",
                btn: ['确定', '关闭'],
                yes: function (index, layero) {
					...
					<c:if test="${callback != null and callback != ''}">
						// 如果设置callbak，自定义后续处理步骤，则执行
						try{${callback}(item);}catch (e) {}
                    </c:if>
