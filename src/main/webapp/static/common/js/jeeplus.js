...
(function () {
    ...
    jp = {
        playErrorAudio() {
            var audio = document.getElementById('__error-audio');
            if (audio) {
        ...
        playWarnAudio() {
            var audio = document.getElementById('__warn-audio');
            if (audio) {
                var playPromise = audio.play();
                if (playPromise !== undefined) {
                    playPromise.then(_ => {
        ...
        playInfoAudio() {
            var audio = document.getElementById('__info-audio');
            if (audio) {
                var playPromise = audio.play();
                if (playPromise !== undefined) {
                    playPromise.then(_ => {
        ...
        playSuccessAudio() {
            var audio = document.getElementById('__success-audio');
            if (audio) {
                var playPromise = audio.play();
                if (playPromise !== undefined) {
                    playPromise.then(_ => {
        ...
        jsonToFormData: function (row) {
            var newRow = {};
            for (var p in row) {
                if (jQuery.isPlainObject(row[p])) {
                    for (var p2 in row[p]) {
                        newRow[p + '.' + p2] = row[p][p2];
                    }
                } else {
                    newRow[p] = row[p];
                }
        ...
        getDictForSelect: function (data) {
            // console.info(data);
            var ret = [];
            for (var i = 0; i < data.length; i++) {
                var d = data[i];
                ret.push({value: d.value, text: d.label});
            }
            return ret;
        },
        ...
        *    beformSubmit：执行提交操作前执行的方法
        ...
        //打开对话框(添加修改)
        openSaveDialog: function (title, url, width = '800px', height = '500px') {
            var auto = true;//是否使用响应式，使用百分比时，应设置为false
            if (width.indexOf("%") >= 0 || height.indexOf("%") >= 0) {
                auto = false;
            }
        ...
        // 打开对话框(添加修改)
        openSaveAndCloseDialog: function (title, url, width = '800px', height = '500px') {
            var auto = true;//是否使用响应式，使用百分比时，应设置为false
            if (width.indexOf("%") >= 0 || height.indexOf("%") >= 0) {
                auto = false;
            }
            layer.open({
                ...
                btn2: function (index, layero) {
                    var iframeWin = layero.find('iframe')[0]; //得到弹出的窗口对象，执行窗口内iframe页的方法：iframeWin.method();
                    iframeWin.contentWindow.save(true);//调用保存事件，在 弹出页内，需要定义save方法。处理保存事件。
                    return false;
                },
                cancel: function (index) {
                    // try{ location.reload(); }catch (e) { }
        ...
        openSaveAndNextDialog: function (title, url, width = '800px', height = '500px') {
            var auto = true;//是否使用响应式，使用百分比时，应设置为false
            if (width.indexOf("%") >= 0 || height.indexOf("%") >= 0) {
                auto = false;
            }
            top.layer.open({
                ...
                btn2: function (index, layero) {
                    var iframeWin = layero.find('iframe')[0]; //得到弹出的窗口对象，执行窗口内iframe页的方法：iframeWin.method();
                    iframeWin.contentWindow.save(true);// 保存后，继续调用add方法，继续增加
                    return false;
                },
                cancel: function (index) {
                    // try{ location.reload(); }catch (e) { }
        ...
        //打开对话框(打印)
        openPrintDialog: function (title, url, width, height) {
            var auto = true;//是否使用响应式，使用百分比时，应设置为false
            if (width.indexOf("%") >= 0 || height.indexOf("%") >= 0) {
                auto = false;
            }
        ...
        //打开对话框(装箱单设置)
        openSettingPrintBoxDialog: function (title, url, width, height, callback, callback2) {
            var auto = true;//是否使用响应式，使用百分比时，应设置为false
            if (width.indexOf("%") >= 0 || height.indexOf("%") >= 0) {
                auto = false;
            }
            top.layer.open({
                ...
                maxmin: true, //开启最大化最小化按钮
                content: url,
                btn: ['&nbsp;保&nbsp;存&nbsp;设&nbsp;置&nbsp;', '恢复默认值', '关闭'],
                yes: function (index, layero) {
                    var iframeWin = layero.find('iframe')[0].contentWindow; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
                    if (callback) {
                        return callback(iframeWin);
                    }
                },
                btn2: function (index, layero) {
                    var iframeWin = layero.find('iframe')[0].contentWindow; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
                    if (callback2) {
                        return callback2(iframeWin);
                    }
                    return false;
                },
                cancel: function () {
    ...
    // 生产cef（嵌入chrome浏览器），打开开发者调试工具
    $(document).keypress(function (e) {
        if (e.ctrlKey && e.shiftKey && e.keyCode === 11) { // ctrl + shift + k
            jsObj.toggleDevTools();
        }
    });
