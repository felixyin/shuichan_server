...
<head>
	...
	<script type="text/javascript">
		try{
			var notAllowCreate = ${notAllowCreate};
			if (notAllowCreate) {
				var dialogIndex = parent.layer.getFrameIndex(window.name); // 获取窗口索引
				parent.layer.close(dialogIndex);
				jp.error('禁止创建用户，超出了套餐允许的最大用户数量：'+notAllowCreate);
			}
		}catch (e) { }
	</script>
</head>
		...
		<table class="table table-bordered">
		   <tbody>
		         ...
		         <td class="width-35">
					 <c:if test="${isAdmin}">
						 <sys:treeselect id="company" name="company.id" value="${user.company.id}" labelName="company.name" labelValue="${user.company.name}"
										 title="公司" url="/sys/office/treeData?type=2" allowClear="true"  cssClass="form-control required disabled"/>
					 </c:if>
					 <c:if test="${!isAdmin}">
						 <sys:treeselect id="company" name="company.id" value="${user.company.id}" labelName="company.name" labelValue="${user.company.name}"
										 title="公司" url="/sys/office/treeData?type=2" allowClear="true"  disabled="disabled" cssClass="form-control required disabled"/>
					 </c:if>
		      ...
		      <tr>
		         ...
		         <td><input id="newPassword" value="123456" name="newPassword" type="password" maxlength="50" minlength="3" class="form-control ${empty user.id?'required':''}"/>
		         ...
		         <td><input id="confirmNewPassword" value="123456" name="confirmNewPassword" type="password"  class="form-control ${empty user.id?'required':''}" maxlength="50" minlength="3" equalTo="#newPassword"/></td>
		      </tr>
