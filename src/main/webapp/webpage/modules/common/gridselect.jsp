        ...
        $(document).ready(function () {
           ...
           var $table =  $('#table').bootstrapTable({
                ...
                onDblClickRow:function(row, $element, field){
                    // 如果是radio，双击后直接选择
                    <c:if test="${!isMultiSelected}">
                    top.window.$('.layui-layer-btn0').click();
                    </c:if>
                }
            });
            ...
            // 如果设置了addurl，则显示快捷新增按钮
            <c:if test="${addUrl != ''}">
            $("#add").click(function () {
                jp.openSaveDialog('新增客户', "${addUrl}", '800px', '500px');
            });
            </c:if>
        });
            ...
            <form id="searchForm" class="form form-horizontal well clearfix">
                <c:forEach items="${searchLabels}" var="name" varStatus="status" >
                    <div class="col-xs-12 col-sm-6 col-md-4">
                        ...
                        <input name="${searchKeys[status.index]}" maxlength="64" tabindex="${status.index + 1}" class="form-control"/>
                    </div>
                </c:forEach>
                <div class="col-xs-12 col-sm-6 col-md-4">
                    <div style="margin-top:26px">
                        ...
                        <c:if test="${addUrl != ''}">
                            <a id="add" class="btn btn-primary btn-rounded  btn-bordered btn-sm"><i class="fa fa-plus"></i> 新增</a>
                        </c:if>
                    </div>
                </div>
            </form>
