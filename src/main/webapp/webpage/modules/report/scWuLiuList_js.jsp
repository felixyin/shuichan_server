...
<script>
    ...
    $(document).ready(function () {
        ...
        function onEditableShownForOrder(_field, _row, _rowIndex, _$el) {
            if(_row.logistics_id === '-'){ // 汇总行id 为 '-1'
        ...
        function onEditableSaveForOrder(_field, _row, _rowIndex, _oldValue, _$el) {
            ...
            // ajax 请求保存数据
            // console.log(data);
            jp.post('${ctx}/report/scReport/saveWuLiuPayPrice', _row, function (result) {
                if (result && result.success) {
                    // ajax 查询订单数据，更新订单表格
                    jp.success('保存成功');
                } else {
                    jp.error('保存失败，或超级管理员不允许修改客户的物流费用！');
                }
            });
        }
        ...
        $('#scOrderTable').bootstrapTable({
            ...
            url: "${ctx}/report/scReport/findWuLiuReportList",
    ...
    function detailFormatter(index, row) {
        ...
        var diaoStatus = $('#diaoStatus').val();
        $.get("${ctx}/report/scReport/findWuLiuItemReportList?&date="+row.logistics_date+'&factoryName='+row.factory_name+'&diaoStatus='+diaoStatus, function (data) {
...
<script type="text/template" id="scOrderChildrenTpl">//<!--
	<div class="tabs-container">
		<ul class="nav nav-tabs">
				<li class="active"><a data-toggle="tab" href="#tab-{{idx}}-1" aria-expanded="true">客户明细</a></li>
		</ul>
