...
<script>
    $(document).ready(function () {
        ...
        $('#scOrderTable').bootstrapTable({
            ...
            url: "${ctx}/report/scReport/findChuHuoReportList",
    ...
    function detailFormatter(index, row) {
        ...
        $.get("${ctx}/report/scReport/findChuHuoItemList?daiYiStatus=1&specId=" + row.id+"&beginDate="+beginDate+'&endDate='+endDate, function (data) {
        ...
        $.get("${ctx}/report/scReport/findChuHuoItemList?daiYiStatus=2&specId=" + row.id+"&beginDate="+beginDate+'&endDate='+endDate, function (data) {
            var scOrderChild1RowIdx = 0,
                scOrderChild1Tpl = $("#scOrderChild1Tpl").html().replace(/(\/\/\<!\-\-)|(\/\/\-\->)/g, "");

            var data1 = data.rows;
            for (var i = 0; i < data1.length; i++) {
                addRow('#scOrderChild-' + row.id + '-2-List', scOrderChild1RowIdx, scOrderChild1Tpl, data1[i]);
                scOrderChild1RowIdx = scOrderChild1RowIdx + 1;
            }
            if(row.dai_count ===0){
                $('a[href="#tab-' + row.id + '-2"]').click();
            }
        });
        return html;
    }
