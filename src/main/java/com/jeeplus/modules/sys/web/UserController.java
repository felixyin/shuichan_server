...
public class UserController extends BaseController {
     ...
     * 套餐最大用户数量限制配置
     */
    public static final int USER_COUNT_1 = 20;
    public static final int USER_COUNT_2 = 50;
    public static final int USER_COUNT_3 = 100;
    public static final int USER_COUNT_4 = 200;
...
//        }
        if (!UserUtils.getUser().isAdmin()) {
            ...
            user.setCompany(officeDirectly);
        }
    ...
    public String form(User user, Model model) {
        ...
        List<Role> role = systemService.findRole(new Role());
        ...
        if (!admin) {
            ...
            for (int i = 0; i < role.size(); i++) {
                Role r = role.get(i);
                if (r.getEnname().equals("system")) {
                    role.remove(r);
                }
            }
            if ("1".equals(officeDirectly.getType())) { // 1 表示加工厂
                ScPay scPay = scPayService.findUniqueByProperty("office_id", officeDirectly.getId());
                // 校验账号数量
                int count = 0;
                List<Map<String, Object>> maps = userMapper.execSelectSql("select count(1) as `count` from sys_user u where u.company_id = '" + officeDirectly.getId() + "'");
                if (null != maps && !maps.isEmpty()) {
                    Map<String, Object> stringObjectMap = maps.get(0);
                    count = Integer.parseInt(stringObjectMap.get("count").toString());
                }
                if (null != scPay) {
                    Integer payModel = scPay.getPayModel();
                    if (payModel == 1 && count > USER_COUNT_1) { // 1表示 月度套餐
                        model.addAttribute("notAllowCreate", USER_COUNT_1);
                    }
                    if (payModel == 2 && count > USER_COUNT_2) {
                        model.addAttribute("notAllowCreate", USER_COUNT_2);
                    }
                    if (payModel == 3 && count > USER_COUNT_3) {
                        model.addAttribute("notAllowCreate", USER_COUNT_3);
                    }
                    if (payModel == 4 && count > USER_COUNT_4) {
                        model.addAttribute("notAllowCreate", USER_COUNT_4);
                    }
                }
            }
        }
   ...
   /*     List<Role> roleList = Lists.newArrayList();
        ...
        if (null == user.getOffice() || StringUtils.isBlank(user.getOffice().getId())) {
