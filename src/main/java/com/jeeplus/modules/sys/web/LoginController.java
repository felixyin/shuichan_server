...
public class LoginController extends BaseController {
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "username", value = "用户名", required = true, paramType = "query", dataType = "string"),
            ...
            @ApiImplicitParam(name = "fromWhere", value = "接口标志", required = false, paramType = "query", dataType = "string"),
    })
    ...
    public String login(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (StringUtils.isNotBlank(fromWhere)) {
            UserUtils.getSession().setAttribute("fromWhere", fromWhere);
        }

        String password = WebUtils.getCleanParam(request, "password");
        UserUtils.getSession().setAttribute("password", password);
    ...
    public String index(HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        if (!admin) { // 管理员用来管理客户的账号、付款信息
            ...
            if (!activation) {
                Object fromWhere1 = UserUtils.getSession().getAttribute("fromWhere");
                if (null != fromWhere1) {
                    String fromWhere = fromWhere1.toString();
                    if ("desktop".equalsIgnoreCase(fromWhere)) { // 桌面软件登录
                        model.addAttribute("flush", 1);
                    } else if ("web".equalsIgnoreCase(fromWhere)) { // 浏览器登录，用来解决支付问题
                        model.addAttribute("flush", 0);
                    }
                } else {
                    model.addAttribute("flush", 0);
                }
                ...
                model.addAttribute("password", UserUtils.getSession().getAttribute("password"));
        ...
        if (UserUtils.getMenuList().size() == 0) {
            ...
            for (Cookie cookie : cookies) {
                ...
                if ("style".equalsIgnoreCase(cookie.getName())) {
                    indexStyle = cookie.getValue();
                }
            }
            // 要添加自己的风格，复制下面三行即可
            if (StringUtils.isNotEmpty(indexStyle)
                    && "jp".equalsIgnoreCase(indexStyle)) {
