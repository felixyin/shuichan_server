package com.jeeplus.modules.order.excel;
...
public class ExcelLinkage {
    ...
    private void creatExcelNameList(HSSFWorkbook workbook, String hideSheetName) {
        ...
        for (int i = 0; i < province.size(); i++) {
            ...
            if ("湖南".equals(province.get(i))) {
                name = workbook.createName();
                num.add(0, province.get(i));
                num.addAll(hnCity);
                name.setNameName(province.get(i));
    ...
    public void Export() {
        try {
            File file = new File("/Users/fy/workspace/shui_chan/codes/shuichan_server/src/main/java/com/jeeplus/modules/order/excel/excel.xls");
